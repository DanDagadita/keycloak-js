...
export default class Keycloak {
  ...
  #loadDefaultAdapter () {
    ...
    return {
      ...
      accountManagement: async (options) => {
        const accountUrl = this.createAccountUrl(options)
  ...
  #loadCapacitorNativeAdapter () {
    const { App, InAppBrowser } = window.Capacitor.Plugins
    ...
    return {
      login: async (options) => {
        ...
        await new Promise(async (resolve, reject) => {
          ...
          let openBrowserPromise;

          loginListener = await App.addListener('appUrlOpen', (event) => {
            cleanupListeners();

            (async () => {
              if (openBrowserPromise) {
                try {
          ...
          openBrowserPromise = InAppBrowser.openInSystemBrowser({ url: loginUrl, options: browserOptions });
      ...
      logout: async (options) => {
        ...
        await new Promise(async (resolve) => {
          ...
          logoutListener = await App.addListener('appUrlOpen', () => {
            cleanupListeners();
          ...
          openBrowserPromise = InAppBrowser.openInSystemBrowser({ url: logoutUrl, options: browserOptions });
      ...
      register: async (options) => {
        ...
        await new Promise(async (resolve, reject) => {
          ...
          registerListener = await App.addListener('appUrlOpen', (event) => {
            cleanupListeners();
          ...
          openBrowserPromise = InAppBrowser.openInSystemBrowser({ url: registerUrl, options: browserOptions });
      ...
      accountManagement: async (options) => {
        const accountUrl = this.createAccountUrl(options)
        if (typeof accountUrl !== 'undefined') {
          await InAppBrowser.openInSystemBrowser({url: accountUrl, options: browserOptions})
        } else {
          throw new Error('Not supported by the OIDC server')
        }
      },
  ...
  createLoginUrl = async (options) => {
    ...
    if (options?.kcTheme) {
      params.append('kc_theme', options.kcTheme)
    }
