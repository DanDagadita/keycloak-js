...
export default class Keycloak {
  ...
  #loadCapacitorNativeAdapter () {
    ...
    const browserOptions = { android: { showTitle: true, viewStyle: 'FULL_SCREEN' } }
    ...
    return {
      login: async (options) => {
        ...
        await new Promise(async (resolve, reject) => {
          ...
          loginListener = await App.addListener('appUrlOpen', (event) => {
            loginListener.remove()
            const oauth = this.#parseCallback(event.url)
            this.#processCallback(oauth).then(() => {
              resolve()
            }).catch((error) => {
              reject(error)
            })
          ...
          await InAppBrowser.openInSystemBrowser({url: loginUrl, options: browserOptions})
        })
      },
      ...
      logout: async (options) => {
        ...
        await new Promise(async (resolve) => {
          ...
          await InAppBrowser.openInSystemBrowser({url: logoutUrl, options: browserOptions})
        })
      },
      ...
      register: async (options) => {
        ...
        await new Promise(async (resolve, reject) => {
          ...
          await InAppBrowser.openInSystemBrowser({url: registerUrl, options: browserOptions})
        })
      },
